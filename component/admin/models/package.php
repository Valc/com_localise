...
class LocaliseModelPackage extends JModelAdmin
{
	...
	public function getItem($pk = null)
	{
		...
		if (!empty($id))
		{
			...
			if ($xml)
			{
				...
				$package->installationindev = array();

				if ($xml->installationindev)
				{
					foreach ($xml->installationindev->children() as $file)
					{
						$data = (string) $file;

						if ($data)
						{
							$package->translationsindev[] = "installation_$data";
						}
						else
						{
							$package->translationsindev[] = "installation_joomla";
						}
	...
	public function save($data)
	{
		...
		if ($package->standalone)
		{
			...
			if ($data['allowdev'] == '1')
			{
				...
				// Add the installation language files in dev
				if (count($installationindev))
				{
					$installXmlindev = $dom->createElement('installationindev');

					foreach ($installationindev as $translation)
					{
						$fileElementindev = $dom->createElement('filename', $translation . '.ini');
						$installXmlindev->appendChild($fileElementindev);
					}
	...
	public function download($data)
	{
		...
		if (count($administrator))
		{
			...
			$admin_txt .= "\t" . '<description>' . $data['language'] . ' - Administrator language</description>' . "\n";
			$admin_txt .= "\t" . '<files>' . "\n";
	...
	public function downloaddev($data)
	{
		...
		if (count($site))
		{
			...
			// As this is a core package, the main joomla file xx-XX.ini should be in the package
			$ref_path = JPATH_ROOT . '/language/' . $reftag . '/' . $reftag . '.ini';
			$frozen_path = JPATH_ROOT . '/language/' . $data['language'] . '/' . $data['language'] . '.ini';
			$dev_path = $site_dev_path . '/' . $reftag . '.ini';
			$task_path = $site_task_path . '/' . $data['language'] . '.ini';
			...
			if (JFile::exists($frozen_path) && !empty($file_data['contents']))
			{
				$site = array_diff($site, array("joomla"));
				$site_txt .= "\t\t" . '<filename>' . $data['language'] . '.ini</filename>' . "\n";
				$site_package_files[] = array('name' => $data['language'] . '.ini','data' => $file_data['contents']);
			}
			else
			{
				$msg .= JText::sprintf('COM_LOCALISE_MAINFILE_NOT_TRANSLATED', $data['language'] . '.ini', JText::_('JSITE'));
			...
			foreach ($site as $translation)
			{
				$ref_path = JPATH_ROOT . '/language/' . $reftag . '/' . $reftag . '.' . $translation . '.ini';
				$frozen_path = JPATH_ROOT . '/language/' . $data['language'] . '/' . $data['language'] . '.' . $translation . '.ini';
				...
				{
					$site_txt .= "\t\t" . '<filename>' . $data['language'] . '.' . $translation . '.ini</filename>' . "\n";
					$site_package_files[] = array('name' => $data['language'] . '.' . $translation . '.ini', 'data' => $file_data['contents']);
				}
				elseif ($translation != 'joomla')
				{
					$msg .= JText::sprintf('COM_LOCALISE_FILE_NOT_TRANSLATED', $data['language'] . '.' . $translation . '.ini', JText::_('JSITE'));
			...
			elseif (!JFile::exists($path) || empty($languagedata))
			{
				$language_data = file_get_contents($dev_path . '/' . $reftag . '.localise.php');
				$language_data = str_replace($reftag, $data['language'], $language_data);
				$language_data = str_replace($refclassname, $langclassname, $language_data);
			}
		...
		if (count($administrator))
		{
			...
			// As this is a core package, the main joomla file xx-XX.ini should be in the package
			$ref_path = JPATH_ROOT . '/administrator/language/' . $reftag . '/' . $reftag . '.ini';
			$frozen_path = JPATH_ROOT . '/administrator/language/' . $data['language'] . '/' . $data['language'] . '.ini';
			$dev_path = $admin_dev_path . '/' . $reftag . '.ini';
			$task_path = $admin_task_path . '/' . $data['language'] . '.ini';
			$headers_path = $frozen_path;
			$ref_keys_path = '';
			...
			if (JFile::exists($frozen_path) && !empty($file_data['contents']))
			{
				$administrator = array_diff($administrator, array("joomla"));
				$admin_txt .= "\t\t" . '<filename>' . $data['language'] . '.ini</filename>' . "\n";
				$admin_package_files[] = array('name' => $data['language'] . '.ini','data' => $file_data['contents']);
			}
			else
			{
				$msg .= JText::sprintf('COM_LOCALISE_MAINFILE_NOT_TRANSLATED', $data['language'] . '.ini', JText::_('JADMINISTRATOR'));
			...
			{
				$path = JPATH_ROOT . '/administrator/language/' . $data['language'] . '/' . $data['language'] . '.' . $translation . '.ini';
				$ref_path = JPATH_ROOT . '/administrator/language/' . $reftag . '/' . $reftag . '.' . $translation . '.ini';
				$frozen_path = JPATH_ROOT . '/administrator/language/' . $data['language'] . '/' . $data['language'] . '.' . $translation . '.ini';
				...
				{
					$admin_txt .= "\t\t" . '<filename>' . $data['language'] . '.' . $translation . '.ini</filename>' . "\n";
					$admin_package_files[] = array('name' => $data['language'] . '.' . $translation . '.ini','data' => $file_data['contents']);
				}
				elseif ($translation != 'joomla')
				{
					$msg .= JText::sprintf('COM_LOCALISE_FILE_NOT_TRANSLATED', $data['language'] . '.' . $translation . '.ini', JText::_('JADMINISTRATOR'));
